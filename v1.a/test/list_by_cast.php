...
$sqlstr .= " WHERE tag_cast LIKE '%" . $cast . "%' ";
...
$lnk_pageprev = '?cast=' . $cast . '&page=' . (($page==1)?(1):($page-1));
$lnk_pagenext = '?cast=' . $cast . '&page=' . (($page==$total_page)?($total_page):($page+1));
$lnk_pagehead = '?cast=' . $cast . '&page=1';
$lnk_pagelast = '?cast=' . $cast . '&page=' . $total_page;

// 處理各頁之超連結：列出所有頁數 (暫未用到，保留供參考)
$lnk_pagelist = '';
for($i=1; $i<=$page-1; $i++)
{ $lnk_pagelist .= '<a href="?cast=' . $cast . '&page='.$i.'">'.$i.'</a> '; }
$lnk_pagelist .= '[' . $i . '] ';
...
$lnk_pagegoto .= '</select>';
$lnk_pagegoto .= '<input type="hidden" name="cast" value="' . $cast . '">';
$lnk_pagegoto .= '</form>';
...
$html = <<< HEREDOC
<button onclick="location.href='all_cast.php';">返回列表</button>
